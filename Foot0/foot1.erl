...
-define(Next_node, 'hand0@127.0.0.1').
-define(My_node,'foot1@127.0.0.1').
-define(Next_module, hand0).
-define(Name_table, foot1).
-define(Cell_Module, cellfoot1).
-define(Recover_module,recoverfoot1).
-define(Timer,100).
-define(Xsize,1000).
-define(Ysize,800).
...
-define(Pid_screen_def,pid_screen_foot1).
-define(Pid_print_speed_def,pid_print_speed_foot1).
...
if NextNode == ?Next_node -> 
	New_node2 = ?My_node;
true -> New_node2 = ?Next_node
end,
 {reply, new_node_received, {State,New_node2}};
  %{reply, new_node_received, {State,New_node}};

handle_call(stop, _From, {State,NextNode}) ->
  {stop, normal, stopped, {State,NextNode}};

handle_call({restart}, _From, {State, NextNode}) -> send_all_cells(ets:first(?Name_table)),
					  gen_server:call({?Name_table,?My_node},{state,State}),
					{reply, finish, {State,NextNode}};
%%%=================================================================================================================
%%                                   HELPING CALLS
%%%=================================================================================================================
...
run_screen(Xsize,Ysize)->
  Wx = wx:new(),
  Frame = wxFrame:new(Wx, -1, "Foot1", [{size, {Xsize, Ysize}}]),
